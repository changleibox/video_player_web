  ...
  group('VideoPlayer for Web', () {
    ...
    setUp(() async {
      ...
      textureId = await VideoPlayerPlatform.instance.create(
        DataSource(sourceType: DataSourceType.network, uri: 'https://flutter.github.io/assets-for-api-docs/assets/videos/bee.mp4'),
      );
    });
    ...
    test('can set looping', () {
      expect(VideoPlayerPlatform.instance.setLooping(textureId, true), completes);
    });
    ...
    test('can seek to position', () {
      expect(VideoPlayerPlatform.instance.seekTo(textureId, Duration(seconds: 1)), completes);
    });

    test('can get position', () {
      expect(VideoPlayerPlatform.instance.getPosition(textureId), completion(isInstanceOf<Duration>()));
    });

    test('can get video event stream', () {
      expect(VideoPlayerPlatform.instance.videoEventsFor(textureId), isInstanceOf<Stream<VideoEvent>>());
    });

    test('can build view', () {
      expect(VideoPlayerPlatform.instance.buildView(textureId), isInstanceOf<Widget>());
    });
  });
}
